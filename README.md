...
```mermaid
flowchart TB
    U[User] --> UI[Web UI / Chat UI / CLI]
    UI --> API[NodePilot Control Plane API]

    subgraph CP[Windows Control Plane]
        API --> AUTH[Authentication and RBAC]
        AUTH --> LG[LangGraph Supervisor]
        LG --> PLAN[Intent Classification and Planning]
        PLAN --> MCP[MCP Gateway and Tool Registry]
        MCP --> POLICY[Policy and Risk Evaluation]
        POLICY -->|Safe| EXEC[Execution Dispatcher]
        POLICY -->|Approval Required| APPROVAL[Human Approval Workflow]
        APPROVAL --> EXEC
        EXEC --> VALIDATE[Result Validation]
        VALIDATE --> RESPONSE[Response Generator]
        RESPONSE --> AUDIT[Audit and Workflow State]
    end

    EXEC -->|Signed Request| AGENT[Linux Node Agent]

    subgraph LN[Ubuntu Managed Node]
        AGENT --> SYSTEM[System Tools]
        AGENT --> PROCESS[Process Tools]
        AGENT --> DOCKER[Docker Tools]
        AGENT --> SERVICE[Service Tools]
        AGENT --> NETWORK[Network Tools]
        AGENT --> FILES[Filesystem Tools]
        AGENT --> PACKAGES[Package Tools]
    end

    AUDIT --> POSTGRES[(PostgreSQL)]
    AUDIT --> REDIS[(Redis)]
    CP --> OBS[Prometheus / Grafana / Loki / OpenTelemetry]
    LN --> OBS
```
